...
# Latest edits: September 9th, 2025
...
# Versions
# v1: initial script, layer heights, angle, plots of thermo over time
# v2: added more things - organic orientations
# v3: distortions, for loop to iterate over placements
# v4: minimizing RAM usage
...
layer_dir = 2 # 0 - x, 1 - y, 2 - z
supercell = np.array([4, 4, 2])
supcell_text = f"{supercell[0]}x{supercell[1]}x{supercell[2]}"
...
mem_thresh = 1024*1024*1024     # 1 GB
...
#@profile
def analyze_system(basename, dump_file, data_file, output_file, output_csv):
# ------------------ LOAD DATA ------------------
    ...
    tot_org_projs = [[_*[] for _ in range(0,3)] for k in range(0, 10)]
    pb_ys = [_*[] for _ in range(0,10)]
    #tot_org_projs = [[]]*10
    #print(tot_org_projs)
    ...
    for frame in range(pipeline.source.num_frames):
        ...
        if pipeline.source.num_frames - frame < 10:
            pb_ys[pipeline.source.num_frames - frame] = pb_z_coords  # Store raw z positions (for histograms or debugging)
        tilt_angles = []
        ...
        if pipeline.source.num_frames - frame < 10:
            for i in range(0,3):
                tot_org_projs[pipeline.source.num_frames - frame][i] = organic_tilt[:][i]
        ...
        del [data, otree, i_tree, pb_tree, positions, types, delta_ds, nn_distances, layers, tilt_angles]
        gc.collect()
    return data_list, pb_ys, tot_org_projs


for hal in halogen:
    for it, placement in enumerate(sys_names):
        basename = f"R-{placement}-{hal}MBA_{system}_{supcell_text}"
        fpath = f"../../HOIS/HOIS_OUTPUT/{system}/{supcell_text}_{system}/{hal}MBA/"+basename+"/"
        ...
        # Loop memory tracking
        memory_usage = process.memory_info().rss
        loop_memory_usage = memory_usage - base_memory_usage
        print('Current memory usage: ', str(loop_memory_usage / (1024*1024)), ' MegaBytes.')
        if loop_memory_usage > mem_thresh:
            print('Memory exceeded threshold of ', str(mem_thresh / (1024*1024)), ' MegaBytes. Stopping program.')
            exit()

        data_list, pb_ys, tot_org_projs = analyze_system(basename, dump_file, data_file, output_file, output_csv)
        ...
        if save_figs:
            ...
            for column in df.columns[1:]:  # Skip "Timestep"
                ...
                plt.cla()
                plt.clf()
                plt.close('all')  # Close to prevent overlap issues
            ...
            for i, k in enumerate(['x', 'y', 'z']):
                #print(len(tot_org_projs[:], axis=0))
                avg_tots = np.mean(tot_org_projs[:][i], axis=0)
                plt.figure()
        ...
        pipeline = None;
        del [df, df_bha, bha_avgs, pipeline, pb_ys, tot_org_projs, data_list]

        matches = [var for var in globals().keys() if (var.endswith("_indices") or var.endswith("_positions"))]
